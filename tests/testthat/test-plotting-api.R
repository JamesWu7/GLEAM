test_that("additional plotting APIs return ggplot", {
  ...
  p1 <- plot_split_violin(sc, signature = rownames(sc$score)[1], x = "celltype", split.by = "group")
  expect_s3_class(p1, "ggplot")

  p2 <- plot_dot_bar(sc, by = c("group", "celltype"), signature = rownames(sc$score)[1])
  expect_s3_class(p2, "ggplot")

  sc2 <- sc
  sc2$meta$stim <- sc2$meta$group
  sc2$meta$seurat_annotations <- sc2$meta$celltype
  p2b <- plot_dot_bar(sc2, by = c("stim", "seurat_annotations"), signature = rownames(sc2$score)[1])
  expect_s3_class(p2b, "ggplot")

  p3 <- plot_pseudobulk_box(sc, signature = rownames(sc$score)[1], group = "group", sample = "sample", celltype = "celltype")
