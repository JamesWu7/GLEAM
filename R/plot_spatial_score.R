...
plot_spatial_score <- function(
  ...
  dat$pathway_score <- as.numeric(score$score[signature, dat$cell_id])
  ...
  p <- p +
    ...
    ggplot2::coord_fixed() +
    ggplot2::labs(title = paste("Spatial signature:", signature), x = NULL, y = NULL, color = "Signature score") +
